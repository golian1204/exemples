...
from bots.models.bots import TokensTelegram
from bots.serializers.bots import TokensTelegramSerializer


class TokensTelegramList(LoggingMixin, generics.GenericAPIView):
    queryset = TokensTelegram.objects.all()
    serializer_class = TokensTelegramSerializer
...
class TokensTelegramDetail(LoggingMixin, generics.GenericAPIView):
    serializer_class = TokensTelegramSerializer
    permission_classes = (permissions.IsAuthenticated, permissions.IsAdminUser,)

    def get(self, request, pk, format=None):
        try:
            token = TokensTelegram.objects.get(pk=pk)
