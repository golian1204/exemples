...
from mess.serializers.polls import InterviewSerializer


class InterviewList(LoggingMixin, generics.GenericAPIView):
...
class InterviewDetail(LoggingMixin, generics.GenericAPIView):
    serializer_class = InterviewSerializer
    permission_classes = (permissions.IsAuthenticated, permissions.IsAdminUser,)
    ...
    def put(self, request, pk, format=None):
        ...
        serializer = self.get_serializer(interview, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response({'status': 'success', 'message': None, 'data': serializer.data}, status=status.HTTP_200_OK)
