...
from mess.models.smart import Product
from mess.serializers.smart import ProductSerializer
from pprint import pprint

class ProductList(LoggingMixin, generics.GenericAPIView):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            from pprint import pprint
            queryset = Product.objects.all().order_by('category__name', 'price', 'name')
            for item in queryset:
                pprint('{} | {} | {}'.format(item.category.name, item.name, item.price))
...
class ProductDetail(LoggingMixin, generics.GenericAPIView):
    serializer_class = ProductSerializer
    permission_classes = (permissions.IsAuthenticated, permissions.IsAdminUser,)

    def get(self, request, pk, format=None):
        try:
            dialog = Product.objects.get(pk=pk)
