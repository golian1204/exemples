...
from subscriptions.serializers.subs_blank import SubsBlankSerializer


class SubsBlankList(LoggingMixin, generics.GenericAPIView):
    queryset = SubsBlank.objects.all()
    serializer_class = SubsBlankSerializer
...
class SubsBlankDetail(LoggingMixin, generics.GenericAPIView):
    serializer_class = SubsBlankSerializer
    permission_classes = (permissions.IsAuthenticated, permissions.IsAdminUser,)

    def get(self, request, pk, format=None):
        try:
            subs_list = SubsBlank.objects.get(pk=pk)
